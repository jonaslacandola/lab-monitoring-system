...
export function useUpdateTimeOut() {
    ...
    const {isPending: isUpdating, mutate: timeOut} = useMutation({
        mutationFn: ({timedOutAttendance, currentDate}) => updateAttendancesTimeOut(timedOutAttendance, currentDate),
        onSuccess: () => {
